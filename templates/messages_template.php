...
<html>
	<head>
		...
		<script src="<?php echo BASE_URL; ?>/data/jquery.js" type="text/javascript"></script>
	...
	<body>
		...
		<div class="container">
			...
			if(!empty($admin) && $admin->num_rows>0){
				...
				for ($i=0;$i<$admin->num_rows;$i++){
					...
					if( $message['viewed'] == 2 && ( $cur_user['rank'] == "admin" && $message['viewed'] == 2 && isset($visitinguser) && $visitinguser != $cur_user['id'] ) ){
						if( $message['admintype'] == "specific" ){
							$image_id = $message['senderid'];
							if ( !file_exists( "data/usericons/" . $image_id . ".png" ) ){
								$image_id = "default";
							}
							$micon = BASE_URL . "/data/usericons/".$image_id.".png";
						}
						else{
				...
				if ($admins<1) echo "An error occurred."; 
			}
			...
			if(!empty($notifications) && $notifications->num_rows>0){
				...
				if ($note<1) echo "An error occurred."; 
			}
			...
			if(!empty($private) && $private->num_rows>0){
				...
				if ($pm<1) echo "An error occurred."; 
			}
